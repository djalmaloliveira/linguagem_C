...
char calculo(float valor, int lmt){
	...
	if (i>0){
		printf("\nApos retirar %d notas de %d, ainda resta: %.2f reais", i, lmt, valor);	
	}
	return valor;
}
...
main(){
	...
	float valor50 = 0,valor20 = 0;
	
	valor50 = calculo(valor,100);
	printf("\nValor do valor50 %d",valor50);
	valor20 = calculo(valor50,50);
	printf("\nValor do valor20 %d",valor20);
	calculo(valor,20);
	calculo(valor,10);
	calculo(valor,2);
